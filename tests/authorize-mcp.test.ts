...
test('MCP discovers active specialist and rejects disabled Authorize mode before remote auth/model calls',async()=>{
 const transport=new StdioClientTransport({command:process.execPath,args:['--import','tsx','src/server.ts'],env:{PATH:process.env.PATH??'',PINGCLI_PROFILE:'test',AUTHORIZE_ENVIRONMENTS:id,AUTHORIZE_CAPABILITIES:'inspect',P1_ENVIRONMENT_ID:id},stderr:'pipe'});
